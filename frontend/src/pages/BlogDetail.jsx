...
const BlogDetail = () => {
  ...
  const getImageUrl = (img) => {
    ...
    if (img.startsWith("http")) return img; // external
    if (img.startsWith("/uploads")) return `${SERVER_URL}${img}`; // backend path
    return `${SERVER_URL}/uploads/${img}`; // fallback
  };
  ...
  return (
    <div className={`min-h-screen py-24 px-4 ${dark ? "bg-gray-900" : "bg-gray-100"}`}>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className={`max-w-4xl mx-auto rounded-xl shadow-xl p-8 relative ${dark ? "bg-gray-800" : "bg-white"}`}
      >
        {/* Close Button */}
        <button
          onClick={() => navigate(-1)}
        ...
        <div className={`flex flex-wrap gap-6 text-sm mb-6 ${dark ? "text-gray-300" : "text-gray-500"}`}>
          ...
          <div className="flex items-center gap-2">
            <MessageCircle size={16} /> {blog.comments?.length || 0} Comments
          </div>
        </div>
          ...
          <div className={`space-y-4 ${dark ? "bg-gray-800 text-white" : "bg-white"}`}>
            ...
            {blog.comments?.map((c, index) => (
              <motion.div
                ...
                className={`p-4 rounded-lg ${dark ? "bg-gray-700" : "bg-gray-100"}`}
              >
